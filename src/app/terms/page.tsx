...
const t = (key: string): string => {
  const dict: Record<string, Record<string, string>> = {
    ...
    zh: {
      ...
      section1Title: "1. 服务描述",
      section1Text: "IPMobi.net提供由位于马来西亚雪兰莪州莎阿南的物理4G/5G调制解调器支持的专用移动代理端口。每个端口对应一个带有真实运营商SIM卡（Maxis、Celcom或Digi）的物理USB调制解调器。服务以订阅制提供。",
      section2Title: "2. 可接受使用",
      section2Text: "您同意不将我们的服务用于：(a) 马来西亚或国际法律下的任何非法活动；(b) 网络入侵或未经授权访问系统；(c) 发送垃圾邮件、恶意软件或进行拒绝服务攻击；(d) 任何违反您通过我们代理访问的第三方平台服务条款的活动。",
      section3Title: "3. 账户注册",
      section3Text: "注册时您必须提供准确信息。每个账户仅限单一实体使用。您有责任维护凭证的机密性，并对您账户下的所有活动负责。",
      section4Title: "4. 付款与计费",
      section4Text: "订阅费按月计费。通过我们的支付网关处理付款。在购买后14天内，针对未使用服务，退款按个案处理。除非您的套餐另有说明，带宽不限量。",
      section5Title: "5. IP轮换",
      section5Text: "所有套餐均提供手动IP轮换。自动化Pro和企业套餐提供API驱动的自动轮换。我们保留在合理通知下因维护目的轮换IP的权利。",
      section6Title: "6. 责任限制",
      section6Text: "IPMobi.net按'现状'提供服务，不作任何担保。我们对因使用或无法使用我们的服务而产生的任何损害（包括第三方平台上的账户封禁）不承担责任。我们的最大赔偿责任限于前一个月支付的费用。",
      section7Title: "7. 终止",
      section7Text: "任何一方可在提前30天书面通知后终止协议。我们保留立即终止违反本条款的账户的权利。终止后，您的端口将被停用，任何剩余数据将被删除。",
      section8Title: "8. 管辖法律",
      section8Text: "本条款受马来西亚法律管辖。任何争议应在马来西亚雪兰莪州法院解决。",
      contact: "如有疑问，请联系",
      backHome: "← 返回首页",
    },
    ru: {
      title: "Условия предоставления услуг", lastUpdated: "Последнее обновление: 28 апреля 2026 г.",
      intro: "Настоящие Условия предоставления услуг регулируют использование вами платформы IPMobi.net, включая все прокси-сервисы, API и связанную инфраструктуру, предоставляемые GreenTech Network.",
      section1Title: "1. Описание услуг",
      section1Text: "IPMobi.net предоставляет выделенные мобильные прокси-порты на базе физических модемов 4G/5G, расположенных в Шах-Аламе, Селангор, Малайзия. Каждый порт соответствует физическому USB-модему с реальной SIM-картой оператора (Maxis, Celcom или Digi). Услуги предоставляются на основе подписки.",
      section2Title: "2. Допустимое использование",
      section2Text: "Вы соглашаетесь не использовать наши услуги для: (a) любой незаконной деятельности согласно малайзийскому или международному законодательству; (b) сетевого вторжения или несанкционированного доступа к системам; (c) рассылки спама, вредоносного ПО или проведения DoS-атак; (d) деятельности, нарушающей условия сторонних платформ.",
      section3Title: "3. Регистрация учетной записи",
      section3Text: "Вы должны предоставить точную информацию при регистрации. Каждая учетная запись предназначена для одного лица. Вы несете ответственность за конфиденциальность ваших учетных данных и за все действия под вашей учетной записью.",
      section4Title: "4. Платежи и выставление счетов",
      section4Text: "Плата за подписку взимается ежемесячно. Платежи обрабатываются через наш платежный шлюз. Возврат средств рассматривается индивидуально в течение 14 дней с момента покупки. Пропускная способность не ограничена, если иное не указано в вашем тарифе.",
      section5Title: "5. Ротация IP",
      section5Text: "Ручная ротация IP доступна во всех тарифах. Автоматическая ротация через API доступна в тарифах Automation Pro и Enterprise. Мы оставляем за собой право ротации IP для обслуживания с разумным уведомлением.",
      section6Title: "6. Ограничение ответственности",
      section6Text: "IPMobi.net предоставляет услуги 'как есть' без каких-либо гарантий. Мы не несем ответственности за любой ущерб, возникший в результате использования или невозможности использования наших услуг.",
      section7Title: "7. Расторжение",
      section7Text: "Любая сторона может расторгнуть соглашение с письменным уведомлением за 30 дней. Мы оставляем за собой право немедленно расторгнуть договор с аккаунтами, нарушающими настоящие условия.",
      section8Title: "8. Применимое право",
      section8Text: "Настоящие условия регулируются законодательством Малайзии. Любые споры разрешаются в судах Селангора, Малайзия.",
      contact: "По вопросам обращайтесь",
      backHome: "← На главную",
    },
  };
